...
public final class RuntimeParameters implements EmailClientParameters
{
  ...
  // Support defaults.
  private static final String DEFAULT_SUPPORT_NAME = "Oracle of Delphi3";
  ...
  // Whether to apply changes when migrate job is run
  private boolean migrateJobApply = false;
  ...
  {
    // --------------------- Get shared Input Parameters using shared code ---------------------
    ...
    // --------------------- Get service specific Input Parameters from a service enum ---------------------
    // This checks system properties and the environment but not a properties file
    addServiceSpecificInputs(inputProperties);
    ...
    if (MIGRATE_JOB_ENV_FLAG.equals(parm))
    {
      _log.info("Processing migratejob. Parm: " + parm);
      setMigrateJobApply(true);
    }
   ...
   */
  private void addServiceSpecificInputs(Properties inputProperties)
  {
    // Get all the system properties.
    Properties sysProps = System.getProperties();
    // Check for values for each possible variable that is not an environment-only variable.
    for (EnvVar2 envVar : EnvVar2.values())
    {
      if (sysProps.containsKey(envVar.getEnvName()) && !envVar.getEnvName().startsWith(TapisEnv.ENVONLY_KEY_PREFIX))
        inputProperties.setProperty(envVar.getEnvName(), sysProps.getProperty(envVar.getEnvName()));
    }
    ...
    for (EnvVar2 envVar : EnvVar2.values())
    {
      ...
      if (envMap.containsKey(propertyName))
        inputProperties.setProperty(propertyName, envMap.get(envVar.getEnvName()));
      else if (envMap.containsKey(envVar.name()))
        inputProperties.setProperty(propertyName, envMap.get(envVar.name()));
    }
  }
